...
import { aboutTeamSchema, TAboutTeamSchema } from '@/schemas'
...
export const AboutTeamUpdate: FC<Props> = ({ data, isOpen, onClose }) => {
  ...
  } = useForm<TAboutTeamSchema>({ resolver: zodResolver(aboutTeamSchema) })

  const onSubmit = async (data: TAboutTeamSchema) => {
    try {
      const response = await fetch(`/api/pages/about/team`, {
        method: 'PATCH',
      ...
      toast.error('Failed!')
      console.error('Failed to update team section: ', err)
    }
  }
