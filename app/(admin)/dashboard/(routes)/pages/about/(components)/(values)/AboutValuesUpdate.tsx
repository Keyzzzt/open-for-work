...
import { aboutValuesSchema, TAboutValuesSchema } from '@/schemas'
...
export const AboutValuesUpdate: FC<Props> = ({ data, isOpen, onClose }) => {
  ...
  } = useForm<TAboutValuesSchema>({ resolver: zodResolver(aboutValuesSchema) })

  const onSubmit = async (data: TAboutValuesSchema) => {
    try {
      const response = await fetch(`/api/pages/about/values`, {
        method: 'PATCH',
      ...
      toast.error('Failed!')
      console.error('Failed to update values section: ', err)
    }
  }
              ...
              <Dialog.Panel className='relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6'>
                <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
                  <div className='relative mt-2 w-full'>
                    <Dialog.Title>
                      <Heading3 value='About page, Values section.' />
                    </Dialog.Title>
                    <div className='flex flex-col mt-4 p-2 rounded-md border border-borderColor bg-lightGray'>
                      ...
                      <div className='border border-borderColor p-2 rounded-md mt-4'>
                        <Paragraph value='Value #1' />
                        <Input
                          register={register}
                          errors={errors}
                          id='about-value-title-1'
                          placeholder='Title'
                          type='text'
                          rows={4}
                          name='title1'
                      ...
                      <div className='border border-borderColor p-2 rounded-md mt-4'>
                        <Paragraph value='Value #2' />
                        <Input
                          register={register}
                          errors={errors}
                          id='about-value-title-2'
                          placeholder='Title'
                          type='text'
                          rows={4}
                          name='title2'
                      ...
                      <div className='border border-borderColor p-2 rounded-md mt-4'>
                        <Paragraph value='Value #3' />
                        <Input
                          register={register}
                          errors={errors}
                          id='about-value-title-3'
                          placeholder='Title'
                          type='text'
                          rows={4}
                          name='title3'
