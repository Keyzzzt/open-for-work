...
export const BlogPageUpdate: FC<Props> = ({ blog, isOpen, onClose }) => {
  ...
  const onSubmit = async (data: TBlogSchema) => {
    try {
      ...
      toast.error('Failed!')
      console.error('Failed to update blog page: ', err)
    }
  }
                    ...
                    <div className='flex flex-col mt-4 p-2 rounded-md border border-borderColor bg-lightGray'>
                      ...
                      <fieldset className='space-y-4 mt-8'>
                        <InputCheckbox
                          register={register}
                          errors={errors}
                          id='blog-update-disabled'
                          name='disabled'
                          defaultValue={blog.disabled}
                          label='Disable whole page'
                        />{' '}
                        <InputCheckbox
                          register={register}
                          errors={errors}
                          id='blog-update-isAboutSectionDisabled'
                          name='isAboutSectionDisabled'
                          defaultValue={blog.isAboutSectionDisabled}
                          label='Disable blog section on About page'
                        />
                        <InputCheckbox
                          register={register}
                          errors={errors}
                          id='blog-update-isLandingSectionDisabled'
                          name='isLandingSectionDisabled'
                          defaultValue={blog.isLandingSectionDisabled}
                          label='Disable blog section on Landing page'
                        />
                      </fieldset>
                    </div>
                  </div>
