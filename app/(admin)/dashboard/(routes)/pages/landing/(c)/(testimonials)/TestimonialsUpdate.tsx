...
import { testimonialsSchema, TTestimonialsSchema } from '@/schemas'
import { Input } from '@/components/Input'
import { CustomButton } from '@/components/CustomButton'
import { Heading3 } from '@/components/text/Heading3'
import { InputCheckbox } from '@/components/client/InputCheckbox'
import { LandingTestimonials } from '.prisma/client'
...
// TODO after update changes order. Send whole array ? Like with story paragraphs???
export const TestimonialsUpdate: FC<Props> = ({ data, isOpen, onClose }) => {
  ...
  } = useForm<TTestimonialsSchema>({
    resolver: zodResolver(testimonialsSchema),
  })

  const onSubmit = async (data: TTestimonialsSchema) => {
    try {
      const response = await fetch(`/api/pages/landing/testimonials`, {
        method: 'PATCH',
      ...
      toast.error('Failed!')
      console.error('Failed to update testimonials section: ', err)
    }
  }
